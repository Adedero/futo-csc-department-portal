...
const approveResult = async (result) => {
  result.loading = true
  await usePost(
    `/api/hod/approve-result/${result._id}`,
    { method: 'PUT', router, toast, toastOnSuccess: true, toastLife: 200 },
    () => {
      result.isHODApproved = true
...
<template>
  <div class="w-full overflow-auto min-h-[calc(100dvh-17.5rem)]">
    ...
    >
      <Column>
        <template #body="{ data }">
          <Button
            v-if="data.HODDisapproved.isDisapproved"
          ...
          <Dialog v-model:visible="data.isVisible" header="Result Disapproved" class="w-80" modal>
            <div>
              ...
              <p class="whitespace-pre-wrap border rounded-lg p-2 mt-2 text-red-500 border-red-500">
                {{ data.HODDisapproved.reason || 'No reason' }}
              </p>
            </div>
          </Dialog>
        </template>
