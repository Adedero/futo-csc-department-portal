...
const { data: staff } = await useGet('/api/hod/profile', {
  router,
  toast
});
...
const saveEdit = async () => {
  loading.value = true;
  await usePost(`/api/hod/update-profile/${staff.value.userId}/staff`,
...
<template>
  <main
    ...
    <aside
      ...
      <StaffInfoEditor :staff model="hod" @image-change="(url) => staff.image = url" />
    </aside>
