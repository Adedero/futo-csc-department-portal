...
const { data: student } = await useGet(`/api/admin/student-profile/${route.params.id}`, {
  router,
  toast
})
...
const updateName = (data) => {
  ...
  student.value.regNumber = data.regNumber
}
...
const goToRecord = (record) =>
  router.push(`/admin/student-course-registration-details/${record._id}`)
</script>
  ...
  >
    ...
    <aside class="md:col-span-3 lg:col-span-3 xl:col-span-2">
      <EditStudentInfo
        ...
        @image-change="(url) => (student.image = url)"
      />
    </aside>
