...
const { data: staff } = await useGet('/api/admin/profile', {
  router,
  toast
});
...
const saveEdit = async () => {
  loading.value = true;
  await usePost(`/api/admin/update-profile/${staff.value._id}/admin`,
...
<template>
  <main
    ...
    <aside
      ...
      <AdminInfoEditor :staff model="admin" @image-change="(url) => staff.image = url" />
    </aside>
