...
const { data: staff } = await useGet(`/api/admin/staff-profile/${route.params.id}`, {
  router,
  toast
})

const updateDetails = (data) => {
  staff.value.user.name = data.name
  staff.value.staffId = data.staffId
  staff.value.rank = data.rank
  staff.value.isActive = data.isActive
  staff.value.user.title = data.title
}
...
<template>
  ...
  >
    ...
    <aside class="md:overflow-y-auto md:col-span-3 lg:col-span-3 xl:col-span-3">
      <EditStaffInfo
        :staff
        @details-change="updateDetails"
        @image-change="(url) => (staff.user.image = url)"
      />
    </aside>
    ...
    >
      ...
      <!--  <VCard>
        ...
        <div class="max-w-full overflow-x-auto">
          <DataTable :value="staff.results" dataKey="session" tableStyle="min-width: 0">
            ...
            <Column field="courses" header="Harmattan">
              <template #body="slotProps">
                {{ getNumberOfCourses(slotProps.data).har }}
              </template>
</Column>

<Column field="courses" header="Rain">
  <template #body="slotProps">
                {{ getNumberOfCourses(slotProps.data).rain }}
              </template>
</Column>

<Column>
  <template #body>
                <Button label="Details" icon="pi pi-eye" outlined rounded />
              </template>
</Column>
</DataTable>
</div>
