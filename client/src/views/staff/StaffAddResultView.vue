...
const { data } = await useGet(
  `/api/staff/get-registered-students?staffId=${route.params.staffId}&session=${route.params.session}&semester=${route.params.semester}&level=${route.params.level}&course=${route.params.course.split('-').join(' ')}`,
  { router, toast },
  (payload) => {
    if (payload.existing) {
      router.push({
