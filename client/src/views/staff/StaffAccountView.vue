...
const { data: staff } = await useGet('/api/staff/profile', {
  router,
  toast
});
...
const saveEdit = async () => {
  loading.value = true;
  await usePost(`/api/staff/update-profile/${staff.value.userId}/staff`,
...
<template>
  <main
    ...
    <aside
      ...
      <StaffInfoEditor :staff model="staff" @image-change="(url) => staff.image = url" />
    </aside>
