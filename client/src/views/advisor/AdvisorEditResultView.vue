...
const { data } = await useGet(
  `/api/advisor/get-class-result-to-edit?resultId=${route.params.resultId}`,
  { router, toast },
  (payload) => {
    if (payload.approved) {
      ...
      setTimeout(() => {
        router.push(`/advisor/view-result/${payload.resultId}`)
      }, 3000)
      return
    }
...
<template>
  <main v-if="data.students" class="w-full h-full overflow-y-auto p-2">
    ...
    <ResultEditor model="advisor" :payload="data" class="mt-2" />
  </main>
</template>
